...
async function fetchDiscordStatus() {
	try {
		const [KirkaResponse] = await Promise.all([
			fetch(`https://api.kirka.io/api/clans/${userID}`).then((response) =>
				response.json()
			),
		]);

		const kirkaData = kirkaResponse.data;

		const { discord_status, activities, discord_user, emoji } = kirkaData;

		elements.Name.innerHTML = discord_user.name;
		elements.allScores.innerHTML = discord_user.allScores;
